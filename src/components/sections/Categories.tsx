    ...
    <section id="sports" className="bg-paper py-14 sm:py-20">
      <div className="container-page">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
          {sports.map((sport, i) => {
            const wide = i === 0 // feature the flagship
            return (
              <Link
                key={sport.id}
                to={`/sports/${sport.id}`}
                className={cn(
                  ...
                  wide ? 'col-span-2 row-span-2 aspect-[4/3] md:aspect-auto' : 'aspect-[4/3]',
                )}
              >
                <SmartImage
                  src={sport.image}
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4 sm:p-5">
                  <div>
                    <p className="byline text-race">{sport.stat.value} · {sport.stat.label}</p>
                    <h3 className={cn('font-mega uppercase leading-none text-white', wide ? 'text-3xl sm:text-5xl' : 'text-xl sm:text-2xl')}>
                      {sport.name}
                    </h3>
                  </div>
